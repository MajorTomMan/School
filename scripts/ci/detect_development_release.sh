...
if git rev-parse -q --verify "refs/tags/$tag" >/dev/null; then
  ...
  if [[ "$VERSION_NAME" != "$previous_name" && "$VERSION_CODE" -gt "$previous_code" ]]; then
    version_changed=true
  fi

  if ! git diff --quiet "refs/tags/$tag..HEAD" -- app visualization build.gradle.kts settings.gradle.kts gradle.properties gradle signing scripts/ci; then
    code_changed=true
  fi
else
  version_changed=true
