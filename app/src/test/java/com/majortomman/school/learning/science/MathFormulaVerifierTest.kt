...
class MathFormulaVerifierTest {
    ...
    fun verifiesEquationAtCustomValues() {
        val result = MathFormulaVerifier.verify(input = "y=2x+1", values = mapOf("x" to 3.0, "y" to 7.0))
        assertEquals(MathFormulaStatus.TRUE_AT_VALUES, result.status)
    }

    @Test
    fun sampleCheckFindsCounterexample() {
        val result = MathFormulaVerifier.verify(input = "x^2=x", sampleRelation = true)
    ...
    @Test
    fun sampleMatchDoesNotClaimProofStatus() {
        val result = MathFormulaVerifier.verify(input = "(x+1)^2=x^2+2x+1", sampleRelation = true)

        assertEquals(MathFormulaStatus.SAMPLE_MATCH, result.status)
        assertTrue(result.samples.isNotEmpty())
        assertTrue(result.samples.all { it.matches })
    }
}
