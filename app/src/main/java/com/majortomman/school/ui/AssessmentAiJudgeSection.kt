...
/** Local deterministic grading and optional AI grading remain independent. */
@Composable
internal fun AssessmentAiJudgeSection(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        AssessmentWorkProcessField(page, dispatch)
                    ...
                    runCatching {
                        ...
                        onSuccess = { result ->
                            if (!result.completed) {
                                ...
                                evaluation = result
                                if (result.answerCorrect) dispatch(AssessmentIntent.SubmitAnswer) else aiRejectedCount += 1
                            }
                        },
                        onFailure = { failure ->
                            aiFailureCount += 1
...
private fun validateAiSettings(settings: AiSettings): String? {
    ...
    return when {
        ...
        !endpoint.startsWith("http://") && !endpoint.startsWith("https://") -> "AI 接口地址格式无效，本次未发起请求，题目仍为未解答"
...
private fun aiFailureMessage(error: Throwable): String = when {
    error is SocketTimeoutException || error.message.orEmpty().contains("timed out", ignoreCase = true) -> "AI 请求超时，本次按未解答处理；可以重试或直接使用本地判题"
    error is IllegalArgumentException -> "AI 协议校验失败：${error.message ?: "返回数据不完整"}；本次按未解答处理"
    else -> "AI 请求失败：${error.message ?: error::class.java.simpleName}；本次按未解答处理"
}

private fun localResultText(page: AssessmentQuestionPageState): String = when (page.progress.latestJudgeResult?.outcome) {
...
private fun CourseAssessmentQuestion.aiPrompt(): String = buildString {
    ...
    if (choices.isNotEmpty()) {
        appendLine("选项：")
        choices.forEach { appendLine("${it.id}：${it.content.joinToString("；") { content -> content.plainText() }}") }
    }
    appendLine("课程标准答案：${referenceAnswer()}")
    explanationText().takeIf(String::isNotBlank)?.let { appendLine("课程编写者参考解析：$it") }
...
private fun CourseAssessmentQuestion.referenceAnswer(): String = when (val rule = definition.answerRule) {
    ...
    is AnswerRule.RationalEquivalent -> if (rule.expected.denominator.toString() == "1") rule.expected.numerator.toString() else "${rule.expected.numerator}/${rule.expected.denominator}"
    ...
    is AnswerRule.Coordinate -> "(${rule.expectedX.asText()}, ${rule.expectedY.asText()})"
}

private fun com.majortomman.school.learning.assessment.domain.RationalValue.asText(): String = if (denominator.toString() == "1") numerator.toString() else "$numerator/$denominator"
private fun CourseAssessmentQuestion.explanationText(): String = explanation.joinToString("；") { it.plainText() }.trim()
...
private fun UserAnswer?.displayText(question: CourseAssessmentQuestion): String = when (this) {
    ...
    is UserAnswer.Choice -> {
        val choiceText = question.choices.firstOrNull { it.id == optionId }?.content?.joinToString("；") { it.plainText() }.orEmpty()
...
private fun LearningContent.plainText(): String = when (this) {
    ...
    is LearningContent.Table -> buildString {
        ...
        rows.forEach { append("；").append(it.joinToString("、")) }
    }
