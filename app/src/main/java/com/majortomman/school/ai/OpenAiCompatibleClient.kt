...
class OpenAiCompatibleClient(
    ...
    private fun sendChat(
        ...
        connection.outputStream.bufferedWriter(Charsets.UTF_8).use { writer -> writer.write(request.toString()) }
    ...
    private fun openConnection(path: String, method: String): HttpURLConnection {
        ...
        require(endpoint.startsWith("http://") || endpoint.startsWith("https://")) { "接口地址必须以 http:// 或 https:// 开头" }
    ...
    private fun HttpURLConnection.readResponse(): String {
        ...
        if (status !in 200..299) throw IOException("HTTP $status: ${body.take(400).ifBlank { responseMessage }}")
