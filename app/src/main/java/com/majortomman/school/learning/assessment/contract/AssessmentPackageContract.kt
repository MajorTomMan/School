...
object AssessmentPackageContract {
    fun validate(course: CourseDocument, assessments: AssessmentDocument, knowledgePoints: KnowledgePointDocument) {
        val courseId = course.textbook.id
        require(assessments.courseId == courseId) { "assessments.courseId 与 course.json 教材 ID 不一致：${assessments.courseId} != $courseId" }
        require(knowledgePoints.courseId == courseId) { "knowledge-points.courseId 与 course.json 教材 ID 不一致：${knowledgePoints.courseId} != $courseId" }
        val sectionIds = course.chapters.flatMap { it.sections }.map { it.id }.toSet()
    ...
    private fun validateKnowledgePoints(document: KnowledgePointDocument, sectionIds: Set<String>) {
        ...
        document.knowledgePoints.forEach { point ->
            ...
            require(unknownPrerequisites.isEmpty()) { "知识点 ${point.id} 引用了不存在的前置知识：${unknownPrerequisites.joinToString()}" }
            val unknownSections = point.sectionIds.toSet() - sectionIds
            require(unknownSections.isEmpty()) { "知识点 ${point.id} 引用了不存在的小节：${unknownSections.joinToString()}" }
        }
    ...
    private fun validatePlacements(assessments: AssessmentDocument, sectionIds: Set<String>) {
        val knownQuestionSets = assessments.questionSets.map { it.id }.toSet()
        assessments.placements.forEach { placement ->
            require(placement.sectionId in sectionIds) { "题组位置引用了不存在的小节：${placement.sectionId}" }
            val unknownSets = placement.questionSetIds.toSet() - knownQuestionSets
            require(unknownSets.isEmpty()) { "小节 ${placement.sectionId} 引用了不存在的题组：${unknownSets.joinToString()}" }
        }
    ...
    private fun validateQuestionBindings(assessments: AssessmentDocument, knowledgePoints: KnowledgePointDocument) {
        ...
        assessments.questionSets.forEach { questionSet ->
            questionSet.questions.forEach { question ->
                ...
                require(unknown.isEmpty()) { "题目 ${question.definition.key.id} 引用了不存在的知识点：${unknown.joinToString()}" }
            }
        }
    }
    ...
    private fun validateAssets(assessments: AssessmentDocument) {
        ...
        val referenced = assessments.questionSets.flatMap { it.questions }.flatMapTo(linkedSetOf()) { it.referencedAssetIds() }
...
@Suppress("unused")
private fun contractIdAnchors(questionSetId: QuestionSetId, assetId: ContentAssetId) = questionSetId to assetId
