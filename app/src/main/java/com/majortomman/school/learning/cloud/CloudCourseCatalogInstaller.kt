...
/** Converts an APK-validated authored course into the local textbook navigation model. */
internal object CloudCourseCatalogInstaller {
    ...
    fun refreshFromCache(context: Context): Int {
        ...
        activeRoot.listFiles().orEmpty().filter(File::isDirectory).forEach { root ->
            ...
            runCatching { install(context, root, courseFile, CourseDocumentParser.decode(courseFile.readText(Charsets.UTF_8))) }
    ...
    private fun install(context: Context, root: File, courseFile: File, course: CourseDocument) {
        ...
        val subject = SubjectTemplates.findByTitle(textbook.subject) ?: error("课程包包含不受支持的学科：${textbook.subject}")
        ...
        require(lessons.isNotEmpty()) { "课程包不包含可导航课时" }
        ...
        val catalog = TextbookCatalog(
            book = CatalogBook(textbook.id, textbook.title, subject.title, grade, volume, textbook.publisher, textbook.edition),
            lessons = lessons,
        )
        val generated = TextbookCatalogParser.generateLessons(slot, catalog)
        val generatedFile = File(root, GENERATED_LESSONS_PATH)
        generatedFile.parentFile?.mkdirs()
        generatedFile.writeText(JSONObject().put("lessons", JSONArray().apply { generated.forEach { put(it.toJson()) } }).toString(2), Charsets.UTF_8)
        ...
        val manifest = MaterialPackManifest(
            ...
            pdf = MaterialPdfAsset(textbook.pdf.path, pdfSha256, textbook.pdf.pageIndexOffset),
        )
        MaterialLibraryStore.upsert(
            context,
            InstalledTextbook(
                slot = slot,
                pack = InstalledMaterialPack(manifest, root.absolutePath, System.currentTimeMillis(), MaterialLibraryStore.directorySize(root)),
    ...
    private fun CourseLesson.toCatalogLesson(): CatalogLesson {
        ...
        return CatalogLesson(id = id, title = title, pageStart = start, pageEnd = end)
    }

    private fun parseGrade(value: String): Int {
        value.toIntOrNull()?.let { return it.coerceIn(1, 16) }
        val names = listOf("十六" to 16, "十五" to 15, "十四" to 14, "十三" to 13, "十二" to 12, "十一" to 11, "十" to 10, "九" to 9, "八" to 8, "七" to 7, "六" to 6, "五" to 5, "四" to 4, "三" to 3, "二" to 2, "一" to 1)
        return names.firstOrNull { (name, _) -> value.contains(name) }?.second ?: error("课程包缺少有效年级")
    }
